...
> 痛点:隐患排查**靠人工巡检**,一个大工地几百个作业面看不过来、**漏检率高**;整改**靠 Excel + 微信催**、**闭环管不住**。
  ...
  1、某个工作面有几个人没带安全帽/反光背心/手套等，yolo只能检测有没有和位置，但如果是人手拿安全帽，或者放在桌子上，就很难得到这些空间语义信息
  2、 传统视觉模型检测**只能查固定类别**(有没有安全帽),但真正危险的是**开放/组合语义**('临边有没有防护''脚手板有没有满铺'’施工现场杂乱、圈出影响通行的位置‘),固定模型覆盖不了。
  3、传统视觉模型需要打标签，成本高，而LISA属于开放语义，少量标注微调可以有不错的效果，可拓展性更强
...
- **核心理由**:本场景选 LISA,不是因为它比 YOLO 更会检测安全帽,而是因为施工隐患是"自然语言指令 + 组合语义 + 像素级定位"问题。
  1. **推理分割**:如"圈出没戴安全帽的工人""圈出未防护临边",包含否定、实体关系和场景判断,不是单一类别检测。
  2. **像素级证据**:LISA 输出 mask,可用于面积/长度估算、整改工单可视化、复查前后对比;通用 VLM 通常只能给描述或粗框。
  3. **Agent 工具化**:LISA 只负责 `图片 + 隐患指令 -> mask + 坐标`,违规判定、复核、工单和闭环交给 Agent 其他工具。
  4. **扩展成本低**:新增隐患先换自然语言指令测试,效果不够再少量 LoRA;比每类隐患单独重训检测器更适合长尾场景。
- **三个救法**:
...
- **围绕现有数据也能举出的"关系/状态/否定语义"例子**:
  1. **手里拿着安全帽,但头上没戴**:YOLO 可能同时检测到 `person` 和 `helmet`,但业务问题不是"图里有没有 helmet",而是"这个工人是否正确佩戴在头上"。这需要 **人-头部-安全帽** 的关系判断。
  2. **安全帽挂在腰间/放在脚边**:图里确实出现了安全帽,但不能据此判定合规。真正隐患是"谁没戴",不是"现场有没有帽子"。
  3. **多人同框,部分戴了、部分没戴**:检测器能出多个 `person` 和多个 `helmet`,但"哪一个人没戴"要靠后处理匹配;遮挡、角度复杂时规则容易错配。
  4. **安全帽戴法不规范**:如歪戴、未系下颌带,已经从简单目标检测变成状态判断。
  5. **反光衣拿在手里/披在肩上**:检测器能看到 vest,但业务要求是"是否正确穿戴在人员身上",本质仍是关系与状态问题。
  6. **多人拥挤、反光衣只露出一部分**:单纯框检测容易不稳,尤其是要回答"哪位工人没穿"时,关系推理难度明显上升。
...
3. **"选 LISA 哪个版本?"** → 见 §5 Q5;主选 `xinlai/LISA-13B-llama2-v1`,A100 40GB 能跑,兼顾效果和代码兼容。
4. **"数据从哪来、怎么保证质量?"** → 见 §6.1;SAM+LLM 合成 + 面积/IoU 过滤 + 人工抽检。
5. **"效果怎么衡量、有没有 bad case?"** → 一定要能说出失败案例和分析。面试官最反感只报好数字的人。
