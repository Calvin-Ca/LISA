...
- [x] 当前纯逻辑测试累计 65 项：包含制品发布、HTTP 指标、滚动分位数、
  告警阈值、Prometheus 输出、mask 计数、监控验收工具和 HTTP 请求体限制
  及其真实容器验收工具测试。
...
当前决定暂不实现和执行，保留以下目的与未来操作方案。

### 目的

- 避免未经小流量验证的新模型一次性影响全部生产请求。
- 让稳定版本和候选版本的镜像、权重、配置和指标均可追踪。
- 在候选版本无法 ready、延迟或错误率退化、CUDA OOM 等异常发生时，能够
  在明确时间内恢复上一稳定版本。
- 提前验证切换、故障检测、显存释放、服务恢复和操作记录形成完整闭环，而
  不是只准备一个未经实测的回滚命令。

### 当前单 GPU 约束

监控验收中单个 LISA 容器整卡峰值显存为 31,752 MiB，服务器还有约
2.3 GiB 的 `bge-m3` 常驻服务。A100 总显存为 40 GiB，因此不能在同一张
GPU 上同时常驻 stable 和 candidate 两个 bf16 LISA 实例，也不能在当前
机器上直接实现真实的 95%/5%、80%/20% 双模型加权灰度。

在增加第二张 GPU 或第二台服务器前，未来应先做“单 GPU 串行发布与自动
回滚演练”。这种方式一次只运行一个 LISA 实例，会有模型重新加载造成的
短暂切换窗口，不应宣称为零停机灰度。

### 未来执行步骤：单 GPU 串行发布与回滚

1. 冻结 stable 的 Docker Image ID、模型 `SHA256SUMS`、Git commit、CLIP
   snapshot、精度、队列和超时配置，不能只记录可变 Docker tag。
2. 为 stable 和 candidate 使用不同模型版本标识，所有响应和指标必须能
   判断当前活动版本。
3. 实现 `start-stable`、`deploy-candidate`、`rollback` 和 `status` 操作；
   同一时刻只允许一个 LISA 容器占用 GPU。
4. 使用独立预发布端口启动 stable，等待 `/ready`，执行真实 mask 冒烟并
   保存基线。
5. 停止 stable，确认其显存释放后启动 candidate；等待 ready 并执行至少
   5 次真实推理。
6. 在隔离实验中主动停止 candidate，模拟候选服务退出；不得停止或干扰
   共享的 `bge-m3`。
7. 故障检测器确认 candidate 不可用后，自动重新启动已冻结的 stable。
8. 等待 stable 恢复 ready，再次验证模型版本、HTTP 响应和 PNG mask。
9. 记录发布耗时、不可用窗口、回滚耗时、失败请求、GPU 峰值、停止后显存
   漂移、共享进程 PID 和脱敏日志。
10. 演练通过后，才允许将同一机制用于真实候选模型。

### 未来执行步骤：真实百分比灰度

只有具备第二张 GPU 或第二台服务器后，才同时运行 stable 和 candidate，
通过网关依次执行：

1. stable 95% / candidate 5%；
2. stable 80% / candidate 20%；
3. stable 0% / candidate 100%。

每一阶段都需要固定观察窗口，比较成功率、P95/P99、队列、CUDA OOM、空
mask、多 mask 和人工业务抽检。任一准入项失败时立即将 candidate 权重降为
0，并确认全部流量恢复到 stable。

### 回滚演练准入条件

- stable 在发布前和回滚后均 ready，并能返回正确版本和有效 PNG mask。
- candidate 能正常启动和推理，模拟故障能够被识别。
- candidate 故障后 stable 在 60 秒内恢复 ready。
- 任一时刻最多一个 LISA 实例占用当前 GPU，GPU 推理最大在途数仍为 1。
- GPU 峰值不超过 36,864 MiB，停止后显存漂移绝对值不超过 500 MiB。
- CUDA OOM 和隐性 GPU 并发为 0，原有 `bge-m3` PID 保持存活。
- 日志和结构化输出不包含 API Key、图片 Base64、Prompt 或私有路径。
- 发布、故障、回滚和恢复均生成可追溯记录。

### 待办

- [ ] （暂缓）保留当前线上模型及完整启动配置。
- [ ] （暂缓）为 stable 和 candidate 使用不同模型版本标识。
- [ ] （暂缓）实现单 GPU 串行发布与自动回滚工具。
- [ ] （暂缓）在预发布环境完成故障和回滚演练。
- [ ] （需要第二 GPU/主机）开启影子流量并进行人工抽检。
- [ ] （需要第二 GPU/主机）依次完成 5%、20% 和 100% 灰度。
- [ ] （暂缓）准备一键切换到上一稳定版本的回滚配置。
- [ ] （暂缓）实现并验证自动回滚条件。
...
- [x] 在 ASGI 层按 Content-Length 和实际接收字节限制完整 HTTP 请求体，
  默认上限 30 MiB；超限返回 HTTP 413 / `request_too_large`，不会进入
  JSON、Base64 或 GPU 处理。
- [x] 准备 `lisa13b-request-limit-v1` 真实 Uvicorn 容器验收，覆盖声明长度
  超限、chunked 累计超限、小请求正常校验、精确指标和敏感正文不落日志。
- [x] 根据初次实测修复 chunked 超限异常被 FastAPI 请求体解析转换为通用
  HTTP 400 的问题；中间件现在抑制内部 400 并统一输出受控 413。
- [x] 在远程服务器完成 `lisa13b-request-limit-v1` 修复后复验：65 项测试、
  3/3 HTTP 用例和全部准入项通过，模型加载与运行时请求均为 0。
- [ ] 在正式反向代理配置相同或更小的请求体上限，形成入口与应用双层保护。
- [x] 增加 JPEG/PNG 签名和头部尺寸预检，在 OpenCV 完整解码前拒绝像素数超限图片。
- [x] 使用 OpenCV 解码，并拒绝无法作为图片解码的内容。
- [x] 对服务接口增加可选 API Key 身份认证。
- [ ] 增加持久化审计日志。
- [x] 实现不区分用户的分割结果持久化和最终点赞/点踩反馈；远程常驻容器升级
  与持久化卷验收仍待执行。
- [ ] 定期扫描镜像和依赖漏洞。
